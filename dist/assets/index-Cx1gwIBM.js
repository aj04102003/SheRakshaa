(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const l of i.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&s(l)}).observe(document,{childList:!0,subtree:!0});function o(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function s(n){if(n.ep)return;n.ep=!0;const i=o(n);fetch(n.href,i)}})();const t={view:"onboarding",onboardingStep:1,settings:null,notifications:[],sosActive:!1,sosCountdown:0,currentLocation:null,tapCount:0,lastTapTime:0,longPressTimer:null},T="112",N=window.location.hostname==="localhost"?"http://localhost:3000":"/api",M=new Audio("original_iphone.mp3");M.loop=!0;M.volume=1;let p=null,j=null,A=[];const E=["#f43f5e","#3b82f6","#10b981","#f59e0b","#8b5cf6"],h=document.getElementById("app");function D(){p||(p=L.map("people-map",{}).setView([28.6139,77.209],15),L.tileLayer("https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png",{attribution:"OpenStreetMap contributors CARTO",subdomains:"abcd",maxZoom:19}).addTo(p))}function P(){if(p||D(),p&&t.currentLocation){const{lat:e,lng:a}=t.currentLocation;j&&p.removeLayer(j),j=L.marker([e,a],{icon:L.divIcon({className:"user-location-marker",html:'<div style="background: #f43f5e; width: 20px; height: 20px; border-radius: 50%; border: 3px solid white; box-shadow: 0 2px 8px rgba(0,0,0,0.3);"></div>',iconSize:[20,20],iconAnchor:[10,10]})}).bindPopup("Your Location").addTo(p),z(),p.setView([e,a],15),setTimeout(()=>{p==null||p.invalidateSize()},100)}}function z(){if(A.forEach(s=>{p==null||p.removeLayer(s)}),A=[],!p||!t.currentLocation)return;const{lat:e,lng:a}=t.currentLocation;[{name:"University Campus",lat:e+.01,lng:a-.01},{name:"Police Station",lat:e-.008,lng:a+.012},{name:"Hospital",lat:e+.005,lng:a+.008}].forEach(s=>{const n=L.marker([s.lat,s.lng],{icon:L.divIcon({className:"safe-zone-marker",html:'<div style="background: #6366f1; width: 16px; height: 16px; border-radius: 50%; border: 2px solid white; box-shadow: 0 2px 6px rgba(0,0,0,0.3);"></div>',iconSize:[16,16],iconAnchor:[8,8]})}).bindPopup(s.name).addTo(p);A.push(n)})}function $(){navigator.geolocation?navigator.geolocation.getCurrentPosition(e=>{t.currentLocation={lat:e.coords.latitude,lng:e.coords.longitude},P()},e=>{console.error("Error getting location:",e);let a="Location access denied.";switch(e.code){case e.PERMISSION_DENIED:a="Location permission denied. Please enable location access.";break;case e.POSITION_UNAVAILABLE:a="Location information unavailable.";break;case e.TIMEOUT:a="Location request timed out.";break}const o=document.getElementById("map");o&&(o.innerHTML=`
          ...
          `)}):console.error("Geolocation is not supported by this browser.")}function R(){const e=localStorage.getItem("she_raksha_stealth_v1");e?(t.settings=JSON.parse(e),t.view="dashboard"):(t.view="onboarding",t.onboardingStep=1)}function B(e){localStorage.setItem("she_raksha_stealth_v1",JSON.stringify(e)),t.settings=e,t.view="dashboard",u()}function b(e){t.notifications.unshift(e)}function u(){h&&(h.innerHTML="",t.view==="onboarding"?H():t.view==="fakecall"?U():t.view==="profile"?F():t.view==="notifications"?Z():t.view==="transparency"?Y():(t.view==="dashboard"?_():t.view==="circle"?G():t.view==="people"&&q(),K()))}function H(){var e,a,o;t.onboardingStep===1?(h.innerHTML=`
    ...
    `,(a=document.getElementById("btn-back-step1"))==null||a.addEventListener("click",()=>{t.onboardingStep=1,u()}),(o=document.getElementById("btn-activate"))==null||o.addEventListener("click",()=>{const s=document.querySelectorAll(".c-name"),n=document.querySelectorAll(".c-phone"),i=[];for(let l=0;l<s.length;l++){const c=s[l].value.trim(),r=n[l].value.trim();c&&r&&i.push({name:c,phone:r,avatarColor:E[l%E.length]})}if(i.length<2){const l=document.getElementById("on-error-2");l.innerText="Add at least 2 guardians",l.classList.remove("hidden");return}B({...t.settings,contacts:i})}))}function _(){var v,x,g,y,w,k,m,C;const e=navigator.onLine;h.innerHTML=`
    <div class="flex flex-col h-full animate-in fade-in duration-500">
      <header class="flex justify-between items-center p-6 pb-2">
        <div>
          <h2 class="text-2xl font-bold text-white tracking-tighter mt-1">Hi, ${(v=t.settings)==null?void 0:v.userName.split(" ")[0]}!</h2>
      ...
      <div class="flex-1 overflow-y-auto no-scrollbar px-6 pb-24 mb-10">
        ...
        <div class="flex gap-6 overflow-x-auto no-scrollbar pb-4">
          ${(x=t.settings)==null?void 0:x.contacts.map(d=>`
          <div class="flex flex-col items-center gap-2 flex-shrink-0">
             <div class="w-14 h-14 rounded-full border-2 border-white/20 flex items-center justify-center font-black text-white text-lg" style="background-color: ${d.avatarColor}">
               ${d.name[0]}
             </div>
             <span class="text-[9px] font-bold text-white/60 uppercase tracking-widest">${d.name.split(" ")[0]}</span>
          </div>
        `).join("")}
        </div>
        ...
        <div class="flex-1 flex flex-col items-center justify-center py-10">
          ...
          <div class="mt-6 flex gap-2">
            ${[...Array(5)].map((d,f)=>`
              <div id="tap-indicator-${f}" class="w-2 h-2 rounded-full bg-gray-600 transition-all duration-300"></div>
            `).join("")}
          </div>
          ...
          <div class="mt-8 flex flex-col items-center gap-3">
             <div class="flex -space-x-2">
               ${(g=t.settings)==null?void 0:g.contacts.slice(0,3).map(d=>`<div class="w-8 h-8 rounded-full border-2 border-white/30 glass-dark flex items-center justify-center text-[8px] font-black text-white" style="background-color: ${d.avatarColor}">${d.name[0]}</div>`).join("")}
           </div>
           <p class="text-[10px] font-bold text-white/60 uppercase tracking-widest">Broadcasting to ${(y=t.settings)==null?void 0:y.contacts.length} Guardians</p>
         </div>
       </div>
 ...
 `;const a=document.getElementById("sos-trigger"),o=()=>{const d=Date.now();d-t.lastTapTime>1500&&(t.tapCount=0),t.tapCount++,t.lastTapTime=d;for(let S=0;S<5;S++){const I=document.getElementById(`tap-indicator-${S}`);I&&(S<t.tapCount?I.className="w-2 h-2 rounded-full bg-rose-500 scale-150 shadow-lg shadow-rose-500/50 transition-all duration-300":I.className="w-2 h-2 rounded-full bg-gray-600 transition-all duration-300")}t.tapCount===2&&setTimeout(()=>{t.tapCount===2&&(c(),i())},300),t.tapCount>=5&&(r(),i()),setTimeout(()=>{t.tapCount<5&&i()},1500)},s=()=>{t.longPressTimer=window.setTimeout(()=>{c(),i()},2e3),a==null||a.classList.add("sos-urgent-pulse")},n=()=>{t.longPressTimer&&(window.clearTimeout(t.longPressTimer),t.longPressTimer=null),a==null||a.classList.remove("sos-urgent-pulse")},i=()=>{t.tapCount=0,t.lastTapTime=0;for(let d=0;d<5;d++){const f=document.getElementById(`tap-indicator-${d}`);f&&(f.className="w-2 h-2 rounded-full bg-gray-600 transition-all duration-300")}},l=async()=>{var d,f;try{(await fetch(`${N}/send-sos`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contacts:((d=t.settings)==null?void 0:d.contacts)||[],message:`🆘 EMERGENCY ALERT! ${((f=t.settings)==null?void 0:f.userName)||"Someone"} needs immediate help. Please contact emergency services. This is an automated SOS alert.`,location:t.currentLocation})})).ok&&b({id:String(Date.now()),timestamp:Date.now(),channel:"cloud",summary:"Emergency SMS sent to all contacts!"})}catch(S){console.error("SMS error:",S),b({id:String(Date.now()),timestamp:Date.now(),channel:"error",summary:"SMS failed - trying emergency call"}),r()}},c=async()=>{await l(),O()},r=()=>{window.location.href=`tel:${T}`,b({id:String(Date.now()),timestamp:Date.now(),channel:"cloud",summary:`Calling emergency: ${T}`})};a==null||a.addEventListener("click",o),a==null||a.addEventListener("mousedown",s),a==null||a.addEventListener("touchstart",d=>{d.preventDefault(),s()}),a==null||a.addEventListener("mouseup",n),a==null||a.addEventListener("mouseleave",n),a==null||a.addEventListener("touchend",d=>{d.preventDefault(),n()}),a==null||a.addEventListener("touchcancel",n),(w=document.getElementById("refresh-location"))==null||w.addEventListener("click",()=>{$()}),$(),(k=document.getElementById("ghost-trigger"))==null||k.addEventListener("click",()=>{t.view="fakecall",u()}),(m=document.getElementById("manual-sms"))==null||m.addEventListener("click",()=>{Q()}),(C=document.getElementById("btn-notifications"))==null||C.addEventListener("click",()=>{t.view="notifications",u()})}function G(){var e,a;h.innerHTML=`
  ...
  `,(a=document.getElementById("btn-add-guardian"))==null||a.addEventListener("click",()=>{V()}),document.querySelectorAll(".call-btn").forEach(o=>{o.addEventListener("click",s=>{const n=s.currentTarget.dataset.phone;n&&(window.location.href=`tel:${n}`,b({id:String(Date.now()),timestamp:Date.now(),channel:"cloud",summary:`Calling ${n}`}))})}),document.querySelectorAll(".message-btn").forEach(o=>{o.addEventListener("click",s=>{var l;const n=s.currentTarget.dataset.phone,i=s.currentTarget.dataset.name;if(n&&i){const c=t.currentLocation?` Location: https://maps.google.com/?q=${t.currentLocation.lat},${t.currentLocation.lng}`:" Location unavailable",r=`🆘 EMERGENCY ALERT! ${((l=t.settings)==null?void 0:l.userName)||"Someone"} needs immediate help!${c} Please contact emergency services immediately. This is an automated SOS alert from SheRaksha app.`;window.location.href=`sms:${n}?body=${encodeURIComponent(r)}`,b({id:String(Date.now()),timestamp:Date.now(),channel:"cloud",summary:`Opening emergency message for ${i}`})}})})}function V(){var a,o;const e=document.createElement("div");e.className="fixed inset-0 bg-black/60 backdrop-blur-xl flex items-center justify-center z-50 p-6",e.innerHTML=`
  ...
  `,document.body.appendChild(e),(a=document.getElementById("cancel-add-guardian"))==null||a.addEventListener("click",()=>{document.body.removeChild(e)}),(o=document.getElementById("confirm-add-guardian"))==null||o.addEventListener("click",()=>{var r;const s=document.getElementById("new-guardian-name"),n=document.getElementById("new-guardian-phone"),i=s.value.trim(),l=n.value.trim();if(!i||l.length<10){alert("Please enter valid name and phone number");return}const c={name:i,phone:l,avatarColor:E[((r=t.settings)==null?void 0:r.contacts.length)||0%E.length]};t.settings&&(t.settings.contacts.push(c),B(t.settings)),document.body.removeChild(e),u()})}function q(){var e,a,o;h.innerHTML=`
    <div class="flex flex-col h-full p-8 animate-in slide-in-from-right-8 duration-500">
      <header class="mb-6">
  ...
  `,setTimeout(()=>{t.currentLocation?P():$()},100),(e=document.getElementById("refresh-location-people"))==null||e.addEventListener("click",()=>{$()}),document.querySelectorAll(".safe-place").forEach(s=>{s.addEventListener("click",()=>{const n=s.dataset.place||"Safe zone",i=encodeURIComponent(n+" near me");window.open(`https://www.google.com/maps/search/${i}`,"_blank")})}),(a=document.getElementById("people-helpline"))==null||a.addEventListener("click",()=>{window.location.href=`tel:${T}`}),(o=document.getElementById("people-sos"))==null||o.addEventListener("click",()=>{O()})}function U(){var a,o;h.innerHTML=`
  ...
  `,M.play().catch(()=>{});const e=()=>{M.pause(),M.currentTime=0,t.view="dashboard",u()};(a=document.getElementById("end-call"))==null||a.addEventListener("click",e),(o=document.getElementById("accept-call"))==null||o.addEventListener("click",e)}function Y(){var e;h.innerHTML=`
    <div class="flex flex-col h-full p-8 animate-in slide-in-from-right-8 duration-500">
  ...
  `,(e=document.getElementById("transparency-back"))==null||e.addEventListener("click",()=>{t.view="profile",u()})}function F(){var o,s,n,i,l;if(!t.settings){t.view="onboarding",u();return}const e=t.settings;h.innerHTML=`
    <div class="flex flex-col h-full p-8 animate-in slide-in-from-right-8 duration-500">
      <header class="flex items-center justify-between mb-8">
            ...
            ${e.contacts.map((c,r)=>`
              <div class="glass-dark p-4 rounded-[1.75rem] flex items-center gap-3">
                ...
                <div class="flex-1 space-y-2">
                  <input class="profile-contact-name w-full bg-slate-950/60 border border-slate-800 p-2 rounded-xl text-xs font-bold text-white focus:border-rose-500 outline-none" value="${c.name}" data-index="${r}" />
                  <input class="profile-contact-phone w-full bg-slate-950/60 border border-slate-800 p-2 rounded-xl text-xs font-bold text-white focus:border-rose-500 outline-none" value="${c.phone}" data-index="${r}" />
                </div>
              </div>
            `).join("")}
          </div>
  ...
  `;const a=c=>{const r=document.getElementById("profile-error");r&&(r.textContent=c,r.classList.remove("hidden"))};(o=document.getElementById("profile-back"))==null||o.addEventListener("click",()=>{t.view="dashboard",u()}),(s=document.getElementById("profile-transparency"))==null||s.addEventListener("click",()=>{t.view="transparency",u()}),(n=document.getElementById("profile-reset"))==null||n.addEventListener("click",()=>{confirm("Reset all She Raksha data and onboarding?")&&(localStorage.removeItem("she_raksha_stealth_v1"),location.reload())}),(i=document.getElementById("profile-save"))==null||i.addEventListener("click",()=>{const c=document.getElementById("profile-name"),r=document.getElementById("profile-email");if(!c||!r)return;const v=c.value.trim(),x=r.value.trim();if(!v||!x.includes("@")){a("Enter a valid name and email address.");return}const g=document.querySelectorAll(".profile-contact-name"),y=document.querySelectorAll(".profile-contact-phone"),w=[];for(let m=0;m<g.length;m++){const C=g[m].value.trim(),d=y[m].value.trim();if(C&&d){const f=t.settings.contacts[m];w.push({name:C,phone:d,avatarColor:(f==null?void 0:f.avatarColor)||E[m%E.length]})}}if(w.length<2){a("Keep at least 2 guardians in your circle.");return}B({userName:v,userEmail:x,contacts:w})}),(l=document.getElementById("profile-test-sms"))==null||l.addEventListener("click",async()=>{var r,v,x;const c="+13369103955";if(!((v=(r=t.settings)==null?void 0:r.contacts)!=null&&v.length)){a("Add at least one contact before testing SMS.");return}try{const g=await fetch(`${N}/api/test-sms`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({phone:c,name:(x=t.settings)==null?void 0:x.userName})});if(g.ok)alert("✅ Test SMS sent to Twilio number! Check your phone.");else{const y=await g.json().catch(()=>({}));a(y.error||"Test SMS failed. Check Twilio account verification.")}}catch{a("Unable to reach cloud SMS service. Check your network/server.")}})}function Z(){var e;h.innerHTML=`
    <div class="flex flex-col h-full p-8 animate-in slide-in-from-right-8 duration-500">
  ...
  `,(e=document.getElementById("noti-back"))==null||e.addEventListener("click",()=>{t.view="dashboard",u()})}function K(){var a,o,s,n;const e=document.createElement("nav");e.className="h-20 border-t border-white/5 flex items-center justify-around px-4 mb-10 bg-slate-950/80 backdrop-blur-xl absolute bottom-0 w-full z-30",e.innerHTML=`
  ...
  `,h.appendChild(e),(a=document.getElementById("nav-home"))==null||a.addEventListener("click",()=>{t.view="dashboard",u()}),(o=document.getElementById("nav-circle"))==null||o.addEventListener("click",()=>{t.view="circle",u()}),(s=document.getElementById("nav-people"))==null||s.addEventListener("click",()=>{t.view="people",u()}),(n=document.getElementById("nav-profile"))==null||n.addEventListener("click",()=>{t.view="profile",u()})}function W(){const e=new Audio;return e.volume=1e-5,e.volume===0}function J(){const e=document.createElement("div");e.className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center animate-in fade-in duration-300",e.innerHTML=`
  ...
  `,document.body.appendChild(e);let a=5;const o=e.querySelector(".text-amber-400"),s=window.setInterval(()=>{if(a--,o&&(o.textContent=a>0?`You can cancel within ${a} seconds`:"Cannot cancel - SOS confirmed"),a<=0){window.clearInterval(s);const n=document.getElementById("cancel-sos");n&&n.remove()}},1e3);return new Promise(n=>{const i=document.getElementById("cancel-sos"),l=document.getElementById("confirm-sos");i==null||i.addEventListener("click",()=>{window.clearInterval(s),document.body.removeChild(e),n(!1)}),l==null||l.addEventListener("click",()=>{window.clearInterval(s),document.body.removeChild(e),n(!0)}),setTimeout(()=>{document.body.contains(e)&&(window.clearInterval(s),document.body.removeChild(e),n(!0))},5e3)})}async function O(){const e=document.getElementById("sos-spinner");if(!await J()){b({id:String(Date.now()),timestamp:Date.now(),channel:"error",summary:"SOS cancelled by user."});return}e==null||e.classList.remove("hidden"),e==null||e.classList.add("flex"),W()&&b({id:String(Date.now()),timestamp:Date.now(),channel:"error",summary:"Warning: Device is in silent mode."}),navigator.geolocation.getCurrentPosition(async s=>{var l,c,r,v,x,g,y;const i=`🚨 SHE RAKSHA SOS ALERT 🚨
...
This is an emergency. Please contact me immediately.`;try{const w=await fetch(`${N}/api/send-sos`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({senderName:(l=t.settings)==null?void 0:l.userName,contacts:(c=t.settings)==null?void 0:c.contacts,message:i})});if(w.ok){const k=await w.json();b({id:String(Date.now()),timestamp:Date.now(),channel:"cloud",summary:`✅ SOS sent to ${((v=(r=k.details)==null?void 0:r.success)==null?void 0:v.length)||0} guardians via cloud.`}),((g=(x=k.details)==null?void 0:x.failed)==null?void 0:g.length)>0?alert(`⚠️ Partial Success: ${k.details.success.length} alerts sent. ${k.details.failed.length} failed.

Note: Twilio trial accounts can only send to verified numbers.`):alert("✅ EMERGENCY ALERT SENT: All guardians notified successfully!")}else throw new Error("Backend dispatch failed")}catch{console.warn("Cloud SOS unreachable, fallback to local device SMS."),b({id:String(Date.now()),timestamp:Date.now(),channel:"sms-fallback",summary:"📱 Cloud unavailable. Please manually message your guardians."});const k=(y=t.settings)==null?void 0:y.contacts.map(m=>`${m.name}: ${m.phone}`).join(`
...
Message:
${i}`)}finally{e==null||e.classList.add("hidden"),e==null||e.classList.remove("flex")}},s=>{let n="SOS failed: ";switch(s.code){case s.PERMISSION_DENIED:n+="GPS permission denied.";break;case s.POSITION_UNAVAILABLE:n+="GPS information unavailable.";break;case s.TIMEOUT:n+="GPS request timed out.";break;default:n+="Unknown GPS error.";break}alert("🚫 "+n+" Enable location for accurate emergency response."),b({id:String(Date.now()),timestamp:Date.now(),channel:"error",summary:n}),e==null||e.classList.add("hidden"),e==null||e.classList.remove("flex")})}function Q(){var a;if(!t.settings||!((a=t.settings.contacts)!=null&&a.length)){alert("Add guardians first in Profile.");return}const e=`🆘 EMERGENCY ALERT! ${t.settings.userName||"Someone"} needs immediate help. Please contact emergency services. This is an automated SOS alert.`;t.settings.contacts.forEach((o,s)=>{setTimeout(()=>{window.location.href=`sms:${o.phone}?body=${encodeURIComponent(e)}`},s*500)}),b({id:String(Date.now()),timestamp:Date.now(),channel:"cloud",summary:`Opening message drafts for ${t.settings.contacts.length} contacts`})}R();u();
